...
module AppInsights =
    ...
    let setDataCap subscriptionId rg appInsightsName componentName dataVolumeCap =
        ...
        az (sprintf "resource create --id /subscriptions/%s/resourceGroups/%s/providers/Microsoft.Insights/components/%s/CurrentBillingFeatures/%s -p %s --verbose" subscriptionId rg appInsightsName componentName options) |> ignore